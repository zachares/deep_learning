...
```bash
cd example_code/example_testing
python example_testing.py
```
`example_testing.py` should run without issue.
```bash
cd ../example_training
python learn.py example_learning_config.yml
```
`learn.py` should run without issue (but requires user input during the run)
...
logging_params:
  ...
  logging_dir: <PATH TO WHERE YOU WANT TO LOG MODELS AND TRAINING STATISTICS>
